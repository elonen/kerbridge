...
├── debian/                 the six Debian packages, for the other deployment
...
| `kerbridge-manage` | The operator CLI, and one of two crates that are not services. |
...
## `debian/` - the six packages

The Debian deployment, where the Docker Compose deployment's counterpart is
`deploy/`. One source
package builds six binary packages: `kerbridge-config` (which owns
...
Two things about it are worth knowing before reading the directory.

**It packages, it does not build.** The six programs are static musl artifacts
...
**Two files in here are generated and not committed.** `debian/changelog` is
...
| `*.install` | Five, not six: which staged paths each package takes. The metapackage ships nothing and so has none. |
| `*.service`, `*.logrotate` | Three systemd units and three rotation snippets, one per daemon. Placed by `dh_installsystemd`, which puts them under `/usr/lib` on both supported releases -- the `/lib` alias older debhelpers used is what lintian rejects at error severity. |
| `*.postinst`, `*.postrm` | The group, the two unix users, the `/etc/krb5.conf` include line, and what purge removes. |
| `kerbridge-config.{templates,config}`, `po/` | The seven debconf questions, and the translation template. |
| `*.lintian-overrides` | Two architecture-qualified lines per shipped binary -- so four for `kerbridge-issuerd`, which ships two. A static musl binary reads as a different lintian tag on amd64 than on arm64. |
| `man/` | The three hand-written man pages, one per daemon. The other twelve are generated by `clap_mangen` -- one for `kbconfig`, seven for `kbmanage`, four for `kbsetup` -- and have no committed copy. |
...
| `check-install` | The other half of `make test-deb`: installs what that built, on trixie and noble, and checks that bookworm and jammy refuse `kerbridge-issuerd`. Four throwaway containers, no init in any of them. |
| `check-manifests` | A `make test-fast` gate, with no Docker and no build. It checks the five values that two files each must agree about: the binaries, the package names, the compat level, the version, and the newest `CHANGELOG.md` section. |

## `testbench/`

Three things, and nothing else. `crates/` is the implementation now, so the
...
| Path | What |
|---|---|
| `content/en.toml` | The page's prose. The source; the other ten are translations of it and must carry exactly its keys. |
...
words already exist in eleven languages in
