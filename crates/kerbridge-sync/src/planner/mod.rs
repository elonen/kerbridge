...
//!   than applying the rest, so a first deploy against a realm directory that already
...
pub enum PlanError {
    ...
    /// a first deploy against a realm directory that already holds a same-named object
    ...
    /// realm directory gives them one namespace. Distinct `group_suffix` values
...
pub fn plan_sync(
    ...
    // recoverable only by restoring the realm directory, and a total outage until
    ...
    if desired.users.is_empty() && !cur_users.is_empty() {
        b.plan.alerts.push(Alert::admission(format!(
            "ADMISSION GROUP expansion yields no users at all while {} are synchronized: \
             ...
             broker to cut access immediately; sync will not empty the realm directory in one \
    ...
    for (oid, du) in desired.users.iter() {
        ...
        match cur_users.get(oid.as_str()) {
            ...
            Some(cu) => {
                ...
                // -- so an old name on their files means the realm directory is stale.
                //
