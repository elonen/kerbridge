...
/// The admission-group DN reaches a filter, so it is escaped like any other value.
#[test]
...
/// `kerbridge_core::tls` owns the trust decision and tests it. What this
/// pins is that the broker reaches it through the form that refuses a
/// missing CA, rather than falling back to the OS trust store.
#[test]
fn refuses_ldaps_without_a_configured_ca() {
    assert!(kerbridge_core::tls::client_config(None).is_err());
}

/// The two spellings a delegated request may name a target by, and the one
/// it may not.
#[test]
...
/// A UPN is refused on the wire even though it would resolve: a second
/// mutable spelling arriving as end-user input is attack surface and
/// support load, and the error has to say which spelling to use instead.
#[test]
fn a_target_may_not_be_a_upn_or_a_broken_identity() {
    ...
    assert!(Target::parse("   ").is_err());
    // Recognized by its tag and then refused, rather than falling through
    // to being read as a login name that happens to contain pipes.
    assert!(Target::parse("kb1|entra|sub|extra").is_err());
    assert!(Target::parse("kb1||sub").is_err());
}

/// The login-name filter constrains `objectClass`. Without it a group
/// resolves here and then dies on its missing `userAccountControl` -- a 500
/// where a refusal belongs -- and the value itself reaches a filter, so it
...
/// The caller/target matrix, as the invariant it is written as. The reads
/// either side of it belong to the directory; this is the rule.
#[test]
fn the_caller_target_matrix() {
    ...
    // Caller is the target: today's self-service path, whatever the
    // delegate answer would have been -- it is not consulted.
    ...
    // Caller differs and is in the target's delegate group: the grant is
    // the target's, and the caller is named for the audit line.
    ...
    // Caller differs and is not: the one refusal in the table, and the one
    // that fails silently if it ever stops happening.
    assert!(matches!(verdict(riku(), svc(), false), Err(Denied::NotDelegate)));
}

/// AD returns a DN in whatever case the object was created with, and the
/// two sides of this comparison are reached by different filters. A
/// case-sensitive test here would demand a delegate group of someone who
/// named nobody but themselves.
#[test]
