...
- sync turns a IdP directory object from the same IdP into one.
...
The IdP directory face is behind the `sync` feature, which only `kerbridge-sync`
...
the IdP directory face — and `src/sync/` is the seam they meet the mirror at.
...
`testbench/fixtures/authentik-token/`. The **IdP directory face**
...
- The **subject encoding** — what goes in field 3 of `kb1|<name>|<subject>`.
  ...
  on — a subject the IdP directory cannot be queried by makes the two faces
...
- Reading the IdP directory: the protocol, the credential, the cursors, and which
