...
//!   is `HKLM\Software\KerBridge` on Windows and a *forced* managed preference
//!   on macOS -- the one an MDM profile writes, not anything the user can set.
//!
//! **No secret is stored in either.** The OIDC refresh token lives in the
//! agent process's memory and dies with it; the access token is discarded the
//! moment the ticket comes back. What persists is a URL, two booleans and a
...
pub struct FileConfig {
    ...
    /// Gates the *entire* NTLM-fallback machinery. Defaults on where there is
    /// an NTLM fallback to recover from, which is Windows and nowhere else.
    #[serde(default = "ntlm_fallback_default")]
    pub ntlm_fallback_recovery: bool,
    /// Let the OS's own token store (WAM/WHfB on Windows) issue the broker
    /// token before the browser is tried. Default on; turning it off forces the
    /// browser flow.
    #[serde(default = "yes")]
...
impl Default for FileConfig {
    fn default() -> Self {
        Self {
            broker_url: None,
            grant_for: None,
            expected_working_as: None,
            grant: None,
            ntlm_fallback_recovery: ntlm_fallback_default(),
...
impl Settings {
    ...
    pub fn load() -> Settings {
        ...
        Settings { file, policy, discovered: None }
    }
    ...
    pub fn ntlm_fallback_recovery(&self) -> bool {
        self.policy.ntlm_fallback_recovery.unwrap_or(self.file.ntlm_fallback_recovery)
    }
    ...
    pub fn windows_sign_in(&self) -> bool {
        self.file.windows_sign_in && cfg!(windows)
    }

    pub fn set_windows_sign_in(&mut self, on: bool) {
        self.file.windows_sign_in = on;
    }
...
pub fn autostart_active() -> bool {
    autostart_enabled() || autostart_locked()
}
...
mod tests {
    ...
    fn recording_the_same_expectation_twice_is_not_news() {
        let mut s =
            Settings { file: FileConfig::default(), policy: Policy::default(), discovered: None };
