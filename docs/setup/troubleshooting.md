...
| Every sign-in is rejected, and nothing in the config set looks wrong | **Entra only:** the broker API issues v1 tokens. This is the most common Entra setup failure: [`entra.md`](entra.md#entra-defaults-that-are-wrong-for-kerbridge) |
| A token request fails with `AADSTS500011` | **Entra only:** the broker API has no Application ID URI |
| Windows sign-in fails with a redirect-URI mismatch | **Entra only:** the public client does not have the WAM redirect URI |
| Sync logs 403 on every Graph read | **Entra only:** admin consent was not given for `User.Read.All` / `Group.Read.All` |
...
| A device grant is refused with `account may not authorize a device` | The user is in the admission group, but not in the device-grant group. A device grant needs both. [`device-grants.md`](device-grants.md#who-goes-in-which-group) |
