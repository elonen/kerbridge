...
2. **Read the IdP directory one way** — a read-only **service account** with an API
...
> silently shortened list, and against authentik's IdP directory — where a deletion
...
- **`state: present`** — on the **provider** and the **Role**. These carry the
  ...
  the REST API, so sync could never match a signed-in user to a IdP directory
