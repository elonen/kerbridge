...
- the Entra application registrations
- the admission group KerBridge needs
- the admin consent that makes the sync app usable
- the values the Entra source's `[provider_config]` must carry
...
The `[provider_config]` values, which `./print-provider-config.sh` prints as a TOML
...
| `print-provider-config.sh` | Prints the values as a `[provider_config]` fragment on stdout, and the credential and paste steps on stderr. It writes no file. |
